...
import { Label, Input, Row, Col, FormFeedback } from 'reactstrap';
...
const Step4 = ({ validation }) => {
  ...
  return (
    <>
      ...
      />
      {validation.touched.IDImage && validation.errors.IDImage ? (
        <FormFeedback type="invalid">{validation.errors.IDImage}</FormFeedback>
      ) : null}
      {validation.touched.profileImage && validation.errors.profileImage ? (
        <FormFeedback type="invalid">{validation.errors.profileImage}</FormFeedback>
      ) : null}
      {validation.touched.intro && validation.errors.intro ? (
        <FormFeedback type="invalid">{validation.errors.intro}</FormFeedback>
      ) : null}
    </>
  );
};
