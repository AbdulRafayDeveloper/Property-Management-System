...
import { getFromServer, postToServer } from '../../../helpers/requests';
import { badgeSelection, formatDate } from '../../../helpers/utils';
...
const Application = () => {
	...
	const init = async () => {
		...
		const result = await getFromServer(`applications?${query}`);
		if (result.status) {
			setData(result.data);
	...
	const sendMessage = async (id) => {
		...
		const result = await postToServer(`rooms/${id}`);
		console.log(result);
		if (result.status) {
			navigate('/chat');
		} else {
															...
															<tr>
																<td>
																	<div className="d-flex align-items-center">
																		<div className="flex-shrink-0">
																			<img src={d?.listing?.photos?.[0]} alt="" className="avatar-sm image_src object-fit-cover" />
																		</div>
																		<div className="flex-grow-1 ms-2 ">{d?.listing?.address}</div>
																	</div>
																</td>
																<td>{formatDate(d?.moveInDate)}</td>
																<td>{formatDate(d?.moveOutDate)}</td>
																<td>${d?.listing?.monthlyRent}</td>
																<td>
																	<h5>
																		<Badge color={badgeSelection(d?.status)}>{d?.status}</Badge>
																	</h5>
																</td>
																<td>{formatDate(d?.createdAt)}</td>
																<td>
																	<Button disabled={messageLoading} onClick={()=>sendMessage(d?.listing?.userId)} className="btn btn-soft-info btn-sm" title="Chat">
