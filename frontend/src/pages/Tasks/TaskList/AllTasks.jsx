...
import { Col, Modal, ModalBody, Row, Label, Input, Button, ModalHeader, FormFeedback, Form, Badge } from 'reactstrap';

import { getTaskList, addNewTask, updateTask, deleteTask } from '../../../slices/thunks';
...
const AllTasks = () => {
	...
	const { taskList, isTaskSuccess, error } = useSelector(selectLayoutProperties);
	...
	const columns = useMemo(
		() => [
			...
			//             <img src={import.meta.env.VITE_API_URL + "/images/users/" + item} alt="" className="rounded-circle avatar-xxs" />
			//           </Link>
			//         ))}
					...
					<div className="card" id="tasksList">
							...
							<div className="d-flex align-items-center">
								...
								{/* <div className="flex-shrink-0">
									<div className="d-flex flex-wrap gap-2">
										...
										{isMultiDeleteButton && (
											<button className="btn btn-success" onClick={() => setDeleteModalMulti(true)}>
						...
						<div className="card-body pt-0">
							...
							<table className="table">
								...
								<tbody>
									<tr>
										<td>Upload necessary documents </td>
										<td>
											<h5>
												<Badge color="danger">Unfinished</Badge>
											</h5>
										</td>
										<td>
											<button className="btn btn-sm btn-soft-primary">Upload</button>
										</td>
									</tr>
									<tr>
										<td>Sign lease</td>
									...
									<tr>
										<td>Specify move-in date and time</td>
										<td>
											<h5>
										...
										<td>
											<button className="btn btn-sm btn-soft-primary">Add</button>
										</td>
									</tr>
									<tr>
										<td>Set up payment method</td>
										<td>
											<h5>
												<Badge color="info">Needs to completed</Badge>
											</h5>
										</td>
										<td>
											<button className="btn btn-sm btn-soft-primary">Add</button>
					...
					<ModalBody className="modal-body">
						<Row className="g-3">
							<Col lg={12}>
								...
								/>
								{validation.touched.taskId && validation.errors.taskId ? <FormFeedback type="invalid">{validation.errors.taskId}</FormFeedback> : null}
							</Col>
							...
							<Col lg={12}>
								...
								/>
								{validation.touched.project && validation.errors.project ? <FormFeedback type="invalid">{validation.errors.project}</FormFeedback> : null}
							</Col>
							<Col lg={12}>
								<div>
									...
									/>
									{validation.touched.task && validation.errors.task ? <FormFeedback type="invalid">{validation.errors.task}</FormFeedback> : null}
								</div>
							</Col>
							<Col lg={12}>
								...
								/>
								{validation.touched.creater && validation.errors.creater ? <FormFeedback type="invalid">{validation.errors.creater}</FormFeedback> : null}
							</Col>
												...
												<div className="form-check d-flex align-items-center">
													...
													<Label className="form-check-label d-flex align-items-center" htmlFor={item.imgId}>
														<span className="flex-shrink-0">
															<img src={import.meta.env.VITE_API_URL + '/images/users/' + item.img} alt="" className="avatar-xxs rounded-circle" />{' '}
														</span>
														<span className="flex-grow-1 ms-2">{item.name}</span>
													</Label>
													{validation.touched.subItem && validation.errors.subItem ? <FormFeedback type="invalid">{validation.errors.subItem}</FormFeedback> : null}
												</div>
											</li>
										))}
									</ul>
							...
							<Col lg={6}>
								...
								/>
								{validation.touched.dueDate && validation.errors.dueDate ? <FormFeedback type="invalid">{validation.errors.dueDate}</FormFeedback> : null}
							</Col>
							<Col lg={6}>
								<Label for="ticket-status" className="form-label">
									Status
								</Label>
								<Input name="status" type="select" className="form-select" id="ticket-field" onChange={validation.handleChange} onBlur={validation.handleBlur} value={validation.values.status || ''}>
							...
							<Col lg={12}>
								...
								{validation.touched.priority && validation.errors.priority ? <FormFeedback type="invalid">{validation.errors.priority}</FormFeedback> : null}
							</Col>
						</Row>
					</ModalBody>
