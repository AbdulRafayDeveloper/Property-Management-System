...
const VerticalLayout = (props) => {
	...
	const status = user?.isrenter ? 'renter' : 'landlord';
    ...
    */
	...
	useEffect(() => {
		...
		const initMenu = () => {
			const pathName = import.meta.env.PUBLIC_URL + props.router.location.pathname;
	...
	return (
		<React.Fragment>
			...
			{(navData || []).map((item, key) => {
				console.log(loc.pathname == item.link)
				return (
					...
					<React.Fragment key={key}>
						...
						{item['isHeader'] ? (
                            ...
                            item.user.includes(status) ?
							<li className={`nav-item ${loc.pathname == item.link ? "active" : ""}`}>
